 ...
 * Enforces both of `assertRank`'s rules — the a-z alphabet and no trailing
 * `a` — and `rank.test.mjs` asserts the two never drift apart.
 */
export function isValidRank(rank: string): boolean {
  return RANK_PATTERN.test(rank) && !rank.endsWith(LOWEST);
}
