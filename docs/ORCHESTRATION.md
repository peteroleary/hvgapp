...
|---|---|---|
| **RAM** | 3 concurrent agents overloaded Peter's Mac (2026-08-22) | **Max 2 live agent processes** until ponytail/headroom land and headroom is measured. Queue, don't swarm. |
...
card per squad, one card per live slot (max 2). Source of truth: the boards, not memory.
...
Never assign more than two live cards at once — the Mac chokes at three agents. When a slot
reports done, refill it within the hour. An idle slot with work in Backlog is your failure,
not the agent's.
```
...
steps:
  - id: ping
    action: send_message
    text: "DISPATCH — JUV: post SLOT 1 / SLOT 2 / BUDGET / BLOCKED per docs/ORCHESTRATION.md P9. Max two live cards."
